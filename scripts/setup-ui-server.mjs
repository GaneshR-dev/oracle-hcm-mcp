...
async function testConnection(body) {
  ...
  if (!baseUrl) {
    throw Object.assign(new Error('baseUrl required'), { status: 400 });
  }
  ...
  let res;
  try {
    res = await fetch(probeUrl, { method: 'GET', headers });
  } catch (e) {
    return {
      ok: false,
      baseUrl,
...
function envLocalContent(body) {
  const lines = [
    ...
    `ORACLE_HCM_BASE_URL=${String(body.baseUrl ?? '').replace(/\/+$/, '')}`,
...
const server = http.createServer(async (req, res) => {
  ...
  // CORS not needed (same origin). Bind localhost only.
